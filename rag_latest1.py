...
def get_snippet(text, query, window=50):
    ...
    if idx == -1:
        return text[:window]
    return "..." + text[max(0, idx - window): min(len(text), idx + len(query) + window)] + "..."
...
# Search function
def search_and_answer_ollama(question, username, top_k=5, file_filter=None, date_filter=None):
...
with tab1:
    ...
    if st.button("Get Answer") and question:
        answer = search_and_answer_ollama(question, username, top_k, file_filter, date_filter)
        st.write("### 🤖 Answer:")
        st.write(answer)
